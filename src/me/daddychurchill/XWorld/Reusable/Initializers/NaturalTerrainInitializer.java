...
public class NaturalTerrainInitializer extends AbstractInitializer {
	...
	// protected int specialBlockOdds; // 1/n chance that there is minerals on this
	// level
	// protected int specialsPerLayer; // number of minerals per layer
	// protected int flowerOdds = 6; // 1/n chance that there is a flower on the
	// grass, if there isn't a tree else make some tall grass
	...
	public NaturalTerrainInitializer(AbstractedShape shape) {
		...
		// materialFlower = odds.nextBoolean() ? Material.FLOWER_POPPY :
		// Material.FLOWER_DANDELION;
	...
	protected void renderHere(AbstractWorld world, InitializeChunk chunk, int x, int y, int z, int topsoilThickness,
			int seaLevel) {
		if (y > 0) {
